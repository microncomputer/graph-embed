...
namespace partition {
    ...
    std::vector<std::vector<double>> embedVia(const std::vector<SparseMatrix>& As, //note: there are no current examples using this 
    ...
    std::vector<std::vector<double>> embedViaMultilevel (const std::vector<SparseMatrix>& As,//note: there are no current examples using this 
            const std::vector<SparseMatrix>& P_Ts,
        ...
        if (levelIndex == P_Ts.size()) {
            ...
            std::cout << "embedding layer " << levelIndex+1 << std::endl;
...
#pragma omp parallel for
    ...
    std::vector<std::vector<double>> embedViaMinimization (const SparseMatrix& A, const int d) {//note: there are no current examples using this 
    ...
    void embedViaMinimization (const SparseMatrix& A, const int d, std::vector<std::vector<double>>& new_coords, const int ITER) {
        ...
        // Initializing Base Unit Directions for xi to be differentiated along
        // any sum of these vectors will lie in the unit cube in R^d. 
        ...
        for (int iter=0; iter<ITER; iter++) {
            for (int i=0; i<n; i++) {
                ...
                if (count != 0) {
                    ...
                    if (min_s >= 0) { //min_s is the direction that gives the minimum?
                        for (int k=0; k<d; k++) {
    ...
    std::vector<std::vector<double>> embed (const std::vector<SparseMatrix>& As, 
            const std::vector<SparseMatrix>& P_Ts, //hierarchy of partitions
            const int d) {//this is currently the only embed function from this 
        //file that is used in the example files.
        /* wrapper function for embedMultilevel 
         *
         *
         */
        assert(As.size() == P_Ts.size() + 1);
